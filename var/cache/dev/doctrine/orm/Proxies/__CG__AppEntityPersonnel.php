<?php

namespace Proxies\__CG__\App\Entity;

/**
 * DO NOT EDIT THIS FILE - IT WAS CREATED BY DOCTRINE'S PROXY GENERATOR
 */
class Personnel extends \App\Entity\Personnel implements \Doctrine\ORM\Proxy\InternalProxy
{
    use \Symfony\Component\VarExporter\LazyGhostTrait {
        initializeLazyObject as private;
        setLazyObjectAsInitialized as public __setInitialized;
        isLazyObjectInitialized as private;
        createLazyGhost as private;
        resetLazyObject as private;
    }

    public function __load(): void
    {
        $this->initializeLazyObject();
    }
    

    private const LAZY_OBJECT_PROPERTY_SCOPES = [
        "\0".parent::class."\0".'age' => [parent::class, 'age', null, 16],
        "\0".parent::class."\0".'email' => [parent::class, 'email', null, 16],
        "\0".parent::class."\0".'groupe' => [parent::class, 'groupe', null, 16],
        "\0".parent::class."\0".'heuresMensuelles' => [parent::class, 'heuresMensuelles', null, 16],
        "\0".parent::class."\0".'id' => [parent::class, 'id', null, 16],
        "\0".parent::class."\0".'indisponibilites' => [parent::class, 'indisponibilites', null, 16],
        "\0".parent::class."\0".'jourSemaine' => [parent::class, 'jourSemaine', null, 16],
        "\0".parent::class."\0".'nom' => [parent::class, 'nom', null, 16],
        "\0".parent::class."\0".'password' => [parent::class, 'password', null, 16],
        "\0".parent::class."\0".'plannings' => [parent::class, 'plannings', null, 16],
        "\0".parent::class."\0".'prenom' => [parent::class, 'prenom', null, 16],
        "\0".parent::class."\0".'roles' => [parent::class, 'roles', null, 16],
        "\0".parent::class."\0".'statut' => [parent::class, 'statut', null, 16],
        "\0".parent::class."\0".'typeContrat' => [parent::class, 'typeContrat', null, 16],
        'age' => [parent::class, 'age', null, 16],
        'email' => [parent::class, 'email', null, 16],
        'groupe' => [parent::class, 'groupe', null, 16],
        'heuresMensuelles' => [parent::class, 'heuresMensuelles', null, 16],
        'id' => [parent::class, 'id', null, 16],
        'indisponibilites' => [parent::class, 'indisponibilites', null, 16],
        'jourSemaine' => [parent::class, 'jourSemaine', null, 16],
        'nom' => [parent::class, 'nom', null, 16],
        'password' => [parent::class, 'password', null, 16],
        'plannings' => [parent::class, 'plannings', null, 16],
        'prenom' => [parent::class, 'prenom', null, 16],
        'roles' => [parent::class, 'roles', null, 16],
        'statut' => [parent::class, 'statut', null, 16],
        'typeContrat' => [parent::class, 'typeContrat', null, 16],
    ];

    public function __isInitialized(): bool
    {
        return isset($this->lazyObjectState) && $this->isLazyObjectInitialized();
    }

    public function __serialize(): array
    {
        $properties = (array) $this;
        unset($properties["\0" . self::class . "\0lazyObjectState"]);

        return $properties;
    }
}
